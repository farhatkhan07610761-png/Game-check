...
function renderGames() {
  ...
  if (loadMore) {
    loadMore.style.display =
      visibleGames < filtered.length ? "block" : "none";
  }
}
...
$$("#categories button").forEach(button => {
  button.addEventListener("click", () => {
    $$("#categories button").forEach(btn => btn.classList.remove("active"));
...
function setHP(id, value) {
  ...
  if (parent) {
    const number = parent.querySelector(".hp-number");
...
function fighterNames() {
  ...
  if ($("#fighterName1")) $("#fighterName1").textContent = name1;
  if ($("#fighterName2")) $("#fighterName2").textContent = name2;
...
function attackEffect(side, weapon) {
  ...
  if (weapon === "SWORD") {
    ...
    effect.className =
      "battle-slash " +
      (side === 1 ? "slash-left" : "slash-right");
  } else {
    effect = document.createElement("div");
    effect.className =
      "battle-projectile " +
      (side === 1 ? "projectile-left" : "projectile-right");
  }
...
async function battleRound(round, stats1, stats2) {
  ...
  const types = ["SWORD", "BLASTER", "ROCKET"];
  const weapon = types[round - 1];
  ...
  if (fighter1) {
    fighter1.classList.add("fighter-charge");
    setTimeout(() => fighter1.classList.remove("fighter-charge"), 600);
  }

  if (fighter2) {
    fighter2.classList.add("fighter-charge");
    setTimeout(() => fighter2.classList.remove("fighter-charge"), 600);
  }

  await wait(450);

  const damage = [30, 35, 40][round - 1];

  if (stats1 >= stats2) {
    attackEffect(1, weapon);

    if (fighter2) {
      fighter2.classList.add("fighter-hit");
      setTimeout(() => fighter2.classList.remove("fighter-hit"), 500);
    }

    await wait(250);

    damagePlayer(2, damage);
  } else {
    attackEffect(2, weapon);

    if (fighter1) {
      fighter1.classList.add("fighter-hit");
      setTimeout(() => fighter1.classList.remove("fighter-hit"), 500);
    }

    await wait(250);

    damagePlayer(1, damage);
  }
...
async function startBattle() {
  ...
  const [name1, name2] = fighterNames();

  const gpu1 = getStat("gpu1");
  const cpu1 = getStat("cpu1");
  const ram1 = getStat("ram1");

  const gpu2 = getStat("gpu2");
  const cpu2 = getStat("cpu2");
  const ram2 = getStat("ram2");
  ...
  for (let round = 1; round <= 3; round++) {
    ...
    if (stats1[round - 1] >= stats2[round - 1]) {
      wins1++;
    } else {
      wins2++;
    }
  ...
  const loserName = winner === 1 ? name2 : name1;

  const winnerFighter =
    winner === 1 ? $(".fighter-one") : $(".fighter-two");

  const loserFighter =
    winner === 1 ? $(".fighter-two") : $(".fighter-one");

  if (winnerFighter) winnerFighter.classList.add("fighter-winner");
  if (loserFighter) loserFighter.classList.add("fighter-loser");
  ...
  if (result) {
    result.innerHTML = `
      ...
      <span>${winnerName} defeated ${loserName}</span>
    `;
  ...
  if (button) {
    ...
    button.textContent = "⚔️ FIGHT AGAIN";
  }
}

on("#startBattle", "click", startBattle);
...
function getGameMultiplier(game) {
  return gameMultipliers[game] || .85;
}
...
async function checkPC() {
  ...
  const consoleBox = $("#analysisConsole");
  ...
  for (const [id, label, value] of stages) {
    setStage(id, "active");
  ...
  const gpu = getStat("checkGpu");
  const cpu = getStat("checkCpu");
  const ram = getStat("checkRam");

  const game = $("#checkGame")?.value || "Minecraft";
  ...
  const multiplier = getGameMultiplier(game);
  ...
  if (score >= 90) {
    tier = "S";
    title = "BEAST MODE";
    description = "Your PC is built for serious gaming.";
  } else if (score >= 78) {
    tier = "A";
    title = "HIGH PERFORMANCE";
    description = "Excellent hardware for modern gaming.";
  } else if (score >= 65) {
    tier = "B";
    title = "SOLID GAMER";
    description = "A strong setup for most games.";
  } else if (score >= 50) {
    tier = "C";
    title = "MID RANGE";
    description = "Good for lighter and competitive games.";
  } else {
    tier = "D";
    title = "ENTRY GAMER";
    description = "Best suited for optimized or lighter games.";
  }
  ...
  if (lowest === gpu) bottleneck = "GPU";
  if (lowest === cpu) bottleneck = "CPU";
  if (lowest === ram) bottleneck = "RAM";
  ...
  const scoreElement = $("#resultScore");
  const titleElement = $("#resultTitle");
  const descriptionElement = $("#resultDescription");
  const fpsElement = $("#resultFps");
  const gpuElement = $("#resultGpu");
  const cpuElement = $("#resultCpu");
  const ramElement = $("#resultRam");
  const bottleneckElement = $("#resultBottleneck");
  ...
  if (bottleneckElement) {
    bottleneckElement.textContent = bottleneck;
  }

  animateNumber(scoreElement, score);
  animateNumber(fpsElement, fps);
  animateNumber(gpuElement, gpu);
  animateNumber(cpuElement, cpu);
  animateNumber(ramElement, ram);
...
function applyTheme(theme) {
  ...
  localStorage.setItem("gamecheckTheme", theme);
...
function updateLoginButton() {
  ...
  const user = localStorage.getItem("gamecheckUser");
...
function renderFriends() {
  ...
  if (!friends.length) {
    list.innerHTML = `
      <div class="friend-card">
        ...
        <small>Add your first gaming friend.</small>
      </div>
    `;
    return;
  }
  ...
  $$("[data-remove-friend]").forEach(button => {
    button.addEventListener("click", () => {
      const index = Number(button.dataset.removeFriend);
...
on("#addFriend", "click", () => {
  const name = prompt("Enter your friend's username:");
...
/*
  Safe reveal:
  CSS no longer hides the page if JS fails.
*/
...
if ("IntersectionObserver" in window) {
  ...
  $$(".section,.game-card").forEach(element => {
...
function createParticles() {
  ...
  const amount = window.innerWidth < 650 ? 8 : 18;
